...
function activate(context) {
    ...
    checkNagariInstallation().then(isInstalled => {
        if (!isInstalled) {
            vscode.window.showWarningMessage('Nagari executable not found. Please install Nagari or configure the path in settings.', 'Open Settings').then(selection => {
...
function registerDocumentFormatting(context) {
...
function registerProjectTemplates(context) {
    const createProjectCommand = vscode.commands.registerCommand('nagari.createProject', async () => {
        const projectTypes = [
...
async function createNagariProject(location, name, type) {
...
function getProjectTemplate(type, name) {
    const templates = {
        'Web Application': {
...
function getWebAppTemplate(name) {
...
function getNodeAppTemplate(name) {
...
function getLibraryTemplate(name) {
    return `# ${name} Library
...
function getExampleTemplate(name) {
...
function getPackageJsonTemplate(name, type) {
...
function getIndexHtmlTemplate(name) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
function getReadmeTemplate(name, type) {
    return `# ${name}
