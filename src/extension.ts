...
function registerCommands(context: vscode.ExtensionContext) {
    ...
    // Format command
    const formatCommand = vscode.commands.registerCommand('nagari.format', async (uri?: vscode.Uri) => {
        const filePath = uri?.fsPath || vscode.window.activeTextEditor?.document.fileName;
        if (!filePath || !filePath.endsWith('.nag')) {
    ...
    // Lint command
    const lintCommand = vscode.commands.registerCommand('nagari.lint', async (uri?: vscode.Uri) => {
        const filePath = uri?.fsPath || vscode.window.activeTextEditor?.document.fileName;
        if (!filePath || !filePath.endsWith('.nag')) {
    ...
    context.subscriptions.push(runCommand, buildCommand, transpileCommand, replCommand,
        createProjectCommand, newFileCommand, formatCommand, lintCommand);
}
...
function startLanguageServer(context: vscode.ExtensionContext) {
    ...
    client = new LanguageClient('nagari', 'Nagari Language Server', serverOptions, clientOptions);
